...
tpot_data = pd.read_csv('CH4_Pmof_model - log version.csv', sep=',', dtype=np.float64)
features = tpot_data.drop('log_P(CH4)', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['log_P(CH4)'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -0.017841235550675738
exported_pipeline = make_pipeline(
    PolynomialFeatures(degree=2, include_bias=False, interaction_only=False),
    StackingEstimator(estimator=LinearSVR(C=25.0, dual=False, epsilon=0.0001, loss="squared_epsilon_insensitive", tol=0.001)),
    ExtraTreesRegressor(bootstrap=False, max_features=0.4, min_samples_leaf=2, min_samples_split=6, n_estimators=100)
)

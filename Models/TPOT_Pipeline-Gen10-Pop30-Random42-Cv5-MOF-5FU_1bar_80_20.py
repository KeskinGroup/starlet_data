...
tpot_data = pd.read_excel('5-FU_1BAR.xlsx', dtype=np.float64)
features = tpot_data.drop('5-FU uptake (g/g)', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['5-FU uptake (g/g)'],train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -0.00209781745151475
exported_pipeline = make_pipeline(
    SelectFromModel(estimator=ExtraTreesRegressor(max_features=0.9500000000000001, n_estimators=100), threshold=0.25),
    PolynomialFeatures(degree=2, include_bias=False, interaction_only=False),
    StackingEstimator(estimator=LinearSVR(C=15.0, dual=True, epsilon=0.1, loss="squared_epsilon_insensitive", tol=0.001)),
    RidgeCV()
)

...
tpot_data = pd.read_excel('MTX_1BAR.xlsx', dtype=np.float64)
features = tpot_data.drop('MTX uptake (g/g)', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['MTX uptake (g/g)'],train_size=0.80, test_size=0.20,random_state=42)

# Average CV score on the training set was: -0.006095731938924825
exported_pipeline = make_pipeline(
    StackingEstimator(estimator=RandomForestRegressor(bootstrap=True, max_features=0.4, min_samples_leaf=20, min_samples_split=13, n_estimators=100)),
    RidgeCV()
)

...
tpot_data = pd.read_csv('H2_shuffled_log_MMM_model.csv', sep=',', dtype=np.float64)
features = tpot_data.drop('log_P(MMM)_H2', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['log_P(MMM)_H2'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -3.3716939135021527e-05
exported_pipeline = make_pipeline(
    StackingEstimator(estimator=SGDRegressor(alpha=0.001, eta0=0.01, fit_intercept=True, l1_ratio=0.75, learning_rate="constant", loss="epsilon_insensitive", penalty="elasticnet", power_t=10.0)),
    XGBRegressor(learning_rate=0.5, max_depth=7, min_child_weight=6, n_estimators=100, n_jobs=1, objective="reg:squarederror", subsample=0.9000000000000001, verbosity=0)
)

...
tpot_data = pd.read_csv('H2_N_model.csv', sep=',', dtype=np.float64)
features = tpot_data.drop('N1bar_H2', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['N1bar_H2'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -7.93948022696801e-06
exported_pipeline = make_pipeline(
    RobustScaler(),
    PolynomialFeatures(degree=2, include_bias=False, interaction_only=False),
    StackingEstimator(estimator=RidgeCV()),
    ExtraTreesRegressor(bootstrap=False, max_features=0.7500000000000001, min_samples_leaf=5, min_samples_split=6, n_estimators=100)
)

...
tpot_data = pd.read_csv('N2_Pmof_model - log version.csv', sep=',', dtype=np.float64)
features = tpot_data.drop('log_P(N2)', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['log_P(N2)'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -0.004651092998403631
exported_pipeline = make_pipeline(
    StackingEstimator(estimator=RidgeCV()),
    StackingEstimator(estimator=AdaBoostRegressor(learning_rate=1.0, loss="linear", n_estimators=100)),
    SelectFwe(score_func=f_regression, alpha=0.024),
    ZeroCount(),
    StackingEstimator(estimator=RidgeCV()),
    GradientBoostingRegressor(alpha=0.75, learning_rate=0.1, loss="huber", max_depth=8, max_features=0.35000000000000003, min_samples_leaf=1, min_samples_split=9, n_estimators=100, subsample=0.8500000000000001)
)

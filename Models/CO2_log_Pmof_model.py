...
tpot_data = pd.read_csv('CO2_Pmof_model - log version.csv', sep=',', dtype=np.float64)
features = tpot_data.drop('log_P(CO2)', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['log_P(CO2)'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -0.01716551289789437
exported_pipeline = make_pipeline(
    PolynomialFeatures(degree=2, include_bias=False, interaction_only=False),
    RandomForestRegressor(bootstrap=False, max_features=0.4, min_samples_leaf=3, min_samples_split=15, n_estimators=100)
)

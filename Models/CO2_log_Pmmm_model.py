...
tpot_data = pd.read_csv('CO2_shuffled_log_MMM_model.csv', sep=',', dtype=np.float64)
features = tpot_data.drop('log_P(MMM) - CO2', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['log_P(MMM) - CO2'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -0.00015241667168460502
exported_pipeline = XGBRegressor(learning_rate=0.1, max_depth=10, min_child_weight=2, n_estimators=100, n_jobs=1, objective="reg:squarederror", subsample=0.35000000000000003, verbosity=0)

...
tpot_data = pd.read_csv('CO2_N_model.csv', sep=',', dtype=np.float64)
features = tpot_data.drop('N1bar_CO2', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['N1bar_CO2'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -0.252242744339058
exported_pipeline = make_pipeline(
    PolynomialFeatures(degree=2, include_bias=False, interaction_only=False),
    StackingEstimator(estimator=XGBRegressor(learning_rate=0.1, max_depth=8, min_child_weight=18, n_estimators=100, n_jobs=1, objective="reg:squarederror", subsample=0.45, verbosity=0)),
    ExtraTreesRegressor(bootstrap=False, max_features=0.35000000000000003, min_samples_leaf=5, min_samples_split=14, n_estimators=100)
)

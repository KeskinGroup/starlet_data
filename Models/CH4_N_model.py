...
tpot_data = pd.read_csv('CH4_N_model.csv', sep=',', dtype=np.float64)
features = tpot_data.drop('N1bar_CH4', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['N1bar_CH4'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -0.021785399169604962
exported_pipeline = make_pipeline(
    SelectFwe(score_func=f_regression, alpha=0.039),
    StackingEstimator(estimator=XGBRegressor(learning_rate=0.1, max_depth=7, min_child_weight=2, n_estimators=100, n_jobs=1, objective="reg:squarederror", subsample=0.8, verbosity=0)),
    ExtraTreesRegressor(bootstrap=True, max_features=0.7500000000000001, min_samples_leaf=5, min_samples_split=7, n_estimators=100)
)

...
tpot_data = pd.read_csv('N2_shuffled_log_MMM_model.csv', sep=',', dtype=np.float64)
features = tpot_data.drop('log_P(MMM)_N2', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['log_P(MMM)_N2'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -1.7842589974893354e-05
exported_pipeline = make_pipeline(
    MinMaxScaler(),
    XGBRegressor(learning_rate=0.5, max_depth=8, min_child_weight=15, n_estimators=100, n_jobs=1, objective="reg:squarederror", subsample=0.8500000000000001, verbosity=0)
)

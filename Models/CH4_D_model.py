...
tpot_data = pd.read_csv('CH4_D_model.csv', sep=',', dtype=np.float64)
features = tpot_data.drop('log_D1bar_CH4', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['log_D1bar_CH4'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -0.009778266103714914
exported_pipeline = make_pipeline(
    StackingEstimator(estimator=XGBRegressor(learning_rate=0.1, max_depth=7, min_child_weight=20, n_estimators=100, n_jobs=1, objective="reg:squarederror", subsample=0.9500000000000001, verbosity=0)),
    RandomForestRegressor(bootstrap=False, max_features=0.3, min_samples_leaf=2, min_samples_split=6, n_estimators=100)
)

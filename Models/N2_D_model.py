...
tpot_data = pd.read_csv('N2_D_model.csv', sep=',', dtype=np.float64)
features = tpot_data.drop('log_D1bar_N2', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['log_D1bar_N2'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -0.004060629798221718
exported_pipeline = make_pipeline(
    StandardScaler(),
    StackingEstimator(estimator=RidgeCV()),
    ExtraTreesRegressor(bootstrap=False, max_features=0.6500000000000001, min_samples_leaf=2, min_samples_split=13, n_estimators=100)
)
